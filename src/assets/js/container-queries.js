...
;(function (
    ...
    ;(function (factory) {
        ...
        var SELECTOR_ESCAPED_REGEXP =
            /\.\\:container\\\(((?:[^()]+?|\\\([^()]*\\\))+?)\\\)|\[data-cq~=["']((?:\\.|[^"'])+)["']\]/gi
        var QUERY_REGEXP =
        ...
        function checkMutations(mutations) {
            ...
            arrayFrom(mutations).forEach(function (mutation) {
                addedNodes.push.apply(
                    addedNodes,
                    arrayFrom(mutation.addedNodes).filter(function (node) {
        ...
        function onDomMutate(event) {
            ...
            mutation[(event.type === 'DOMNodeInserted' ? 'added' : 'removed') + 'Nodes'] = [event.target]
        ...
        function parseQuery(query) {
            ...
            var valueType =
                query[5] || values[0].match(NUMBER_REGEXP)
                    ? 'n'
                    : values[0].match(LENGTH_REGEXP)
                    ? 'l'
                    : values[0].match(COLOR_REGEXP)
                    ? 'c'
                    : 's'
            if (valueType === 'n') {
            ...
            return {
                ...
                _filter: query[5],
                _types: [
                    query[6] !== ':' ? query[6] : query[3] === 'min' ? '>=' : query[3] ? '<=' : '=',
                    query[2] &&
                        query[2].replace(/[<>]/, function (match) {
        ...
        function splitSelectors(selectors) {
            return (selectors.match(/(?:\\.|"(?:\\.|[^"])*"|\([^)]*\)|[^,])+/g) || []).map(function (selector) {
        ...
        function buildStyleCacheFromRules(rules) {
            for (var i = 0; i < rules.length; i++) {
                if (rules[i].type === 1) {
                    // Style rule
                    if (rules[i].style.getPropertyValue('width') || rules[i].style.getPropertyValue('height')) {
                        splitSelectors(escapeSelectors(rules[i].selectorText)).forEach(function (selector) {
                            ...
                            rightMostSelector = rightMostSelector.replace(PSEUDO_CLASS_REGEXP, '').trim()
        ...
        function updateClassesRead(treeNodes, dontMarkAsDone) {
            ...
            for (i = 0; i < treeNodes.length; i++) {
                ...
                hasChanges =
                    updateClassesRead(node._children, dontMarkAsDone || node._changes.length) ||
        ...
        function evaluateQuery(parent, query) {
            ...
            if (
                (query._types[0][0] === '>' || query._types[0][0] === '<') &&
                (typeof cValue !== 'number' || typeof qValues[0] !== 'number')
            ) {
                return false
            }
        ...
        function isFixedSize(element, prop) {
            ...
            if (originalStyle && (originalStyle.match(LENGTH_REGEXP) || originalStyle.match(/^calc\([^%]*\)$/i))) {
                return true
            }
            return false
        }
        ...
        function getComputedStyle(element) {
            ...
            if (
                ...
                style.getPropertyValue = function (property) {
                    return this[
                        property.replace(/-+(.)/g, function (match, char) {
        ...
        function getOriginalStyle(element, prop) {
            ...
            matchedRules = sortRulesBySpecificity(filterRulesByElementAndProp(styleCache[prop], element, prop))
        ...
        function filterRulesByElementAndProp(rules, element, prop) {
            ...
            foundRules = foundRules.concat(rules[element.tagName.toLowerCase()] || []).concat(rules['*'] || [])
            return foundRules.filter(function (rule) {
                return (
                    rule._rule.style.getPropertyValue(prop) &&
        ...
        function getSpecificity(selector) {
            ...
            return idScore * 256 * 256 + classScore * 256 + typeScore
        }
        ...
        function hasAttributeValue(element, attr, value) {
            return !!(element.getAttribute(attr) || '').match(
                new RegExp('(?:^|\\s+)' + value.replace(REGEXP_ESCAPE_REGEXP, '\\$&') + '($|\\s+)')
            )
        }
        ...
        function removeAttributeValue(element, attr, value) {
            element.setAttribute(
                attr,
                (element.getAttribute(attr) || '').replace(
                    new RegExp('(?:^|\\s+)' + value.replace(REGEXP_ESCAPE_REGEXP, '\\$&') + '($|\\s+)'),
                    '$1'
                )
            )
